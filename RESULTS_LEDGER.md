...
## 2026-08-17 ~03:35 ET — LEVEL-INTERACTION ORDER: 0 of 4 ORDER-MATTERS. Level-ladder ordering does NOT survive the wall.
- seq_level_order_20260817.md (commit 48375d3). 634 BA + 387 grinder fires, engine S->G->F->C->B->E unchanged, E3 $500, same MINE/HOLD-OUT split, 5000x null on frozen survivors. Exit 0, no traceback.
- H1 LADDER-UP (VWAP-reclaim THEN $-hold): UNDERPOWERED both lanes — the ordered pair occurs on ~0.5% of fires (BA N=1, GR N=2). A FREQUENCY finding: this ladder essentially does not happen before a session-high break.
- H2 PMH-FIRST: MINE OPPOSES its registration in both lanes (BA -$14.19, GR -$9.92); HOLD-OUT sign FLIPS BACK for GR (+$16.15). UNDERPOWERED (BA, hold N=11) / NO-SPLIT (GR). No stable read.
- H3 REJECTION-SCAR: strongest MINE effect in the study and it AGREED with registration (BA -$26.91, GR -$37.73) — then collapsed on HOLD-OUT (BA -$6.42, p=0.304; GR flipped positive). NO-SPLIT / UNDERPOWERED. **A textbook in-sample mirage; the wall did its job.**
...
## 2026-08-17 ~10:20 ET — MORNING BLACKOUT FORENSIC: ROOT CAUSE PROVEN = BELL-BOUNDARY, NOT PRE
- pre_staleness_forensic_20260817.md (commit 2a8951a). **The exact line: _live_sessions (:1254 pre-fix) flips to RTH-only at 09:30:00 SHARP, when zero completed RTH 1-min bars exist yet** -> session filter discards seconds-fresh PRE bars, returns prior-day RTH only -> _fresh_session correctly rejects -> read-list fail-closed fires ROSTER-WIDE (23/26). 3-min probe cache + count=6 tail stretch recovery to 09:36-09:41. Reproduced with frozen clock; rig section R exits 1 on pre-fix code, 0 post-fix.
- CORRECTIONS TO THE MORNING NARRATIVE: (1) PRE itself was NOT dark — the ~223 PRE skips were zero-print names (SQFT/AKTX class), correct refusals; WETO/FIEE/DFSC were never skipped before 09:30. The blackout was the 09:30-09:35 boundary burst. (2) stale_swap/stale_fire are separate mechanisms; stale_fire_suppressed = scan-loop cycle latency at the open (85-195s bursts) — documented open item, cost today ~STFS +$119 forgone.
- Counterfactual cost of the boundary defect today ≈ $0 net (suppressed fires ≈ -$34 net; the STFS winner was latency, not boundary). Real cost = 5 dark roster-wide minutes at EVERY open.
- FIX (coded, committed, NOT deployed): _live_sessions keeps ["PRE","RTH"] for RTH_HANDOFF_MIN (default 5) after the bell; kill switch RTH_HANDOFF_MIN=0; staleness standard untouched.

## 2026-08-17 ~13:00 ET — KEVSEQ RECONCILED: edge is REAL; the DRIFT DEFECT inverted it. My "net-negative" verdict RETRACTED.
- kevseq_reconciliation_20260817.md (commit cde2acf). Marcos challenged the contradiction ("Kevseq was backtested and it proved. How can you now say the opposite????") — he was RIGHT.
- ONE fire-set, E3 live-parity $500, both OOS halves. ARM-SPEC (entry at the detector's fire price) = **+$2.46/tr MINE (N=918), +$3.23/tr HOLD-OUT (N=366)** — POSITIVE ON BOTH HALVES. ARM-LIVE (drifted entry, what the code did) = -$3.54/-$2.46. ARM-F3 (limit-at-fire +0.5%) = -$0.56/-$0.73 universe.
- **THE DRIFT DEFECT IS THE ENTIRE SIGN FLIP: +$6.00/tr MINE, +$5.69/tr HOLD-OUT (169%/231% of the reported deficit)** — and that is a LOWER BOUND (modelled drift median +0.76% vs live-stamped +5.02%).
- ON RUNNER NAMES (fast-chart cohort): SPEC +$13.72/+$13.44, **F3 +$12.77/+$10.58** — the physically realizable arm is STRONGLY POSITIVE on runners, ~break-even at universe scale. Cohort explains ~93% of the original +$340's MAGNITUDE; it does not create the edge (universe SPEC positive both halves).
- **RECORD CORRECTION: entry_drift_20260817.md's "every arm is net-negative" is WRONG AS WRITTEN** — it never graded entry-at-fire-price. My verbal verdict "kevseq is net-negative" is RETRACTED.
- CONVERGENCE (two independent studies, same day): kevseq pays on RUNNERS, not on everything; and the live book put $101 into NIVF (+9.8%, thin) while TRUG/CDTG/SLE/XPON (+27-58%, liquid, all BOT-tagged) went untraded. **THE RANKING HOLE — the bot has no mechanism to prefer the best available candidate; it fills by arrival order.** Never built, never decided-and-lost.
- OWED (Marcos's priced calls): (1) KEVSEQ_LIMIT_ENTRY=1; (2) candidate-ranking queue, kill-test first.

## 2026-08-17 night — FOUNDATION BATCH C: ma_pullback WITHIN-PROCESS DUPLICATION (C1) + CONFIG HASH FREEZE (C2)
- C1 doc: data/killtests/ma_pullback_dup_20260817.md (commit fb67828, Acceptance rig/test_gates_20260817.py::SPEC_ma_pullback_dedupe, PROVEN red-at-parent/green-at-commit). C2 doc: data/audits/config_hash_20260817.md (commit 3a7f881, Acceptance ::SPEC_config_hash_stamp, PROVEN). Harness follow-through f3c5fe5.
- **C1 ROOT CAUSE — STATE, not logging-only.** `detect_ma_pullback` is a PURE FUNCTION of the bar slice; while the confirmation candle stayed the last completed bar it re-returned the identical fire every scan pass, and NOTHING marked the setup consumed. Each pass did not merely re-log: it pushed a fresh candidate into `breakouts` and ran the entire downstream gate + trade path. The lane genuinely re-attempted the trade every cycle. Distinct from the cross-restart replay defect `_fire_once` fixed — this repeats inside one process life. ma_pullback carried no bucket stamp at all, which is why the archive alone could not diagnose it.
- EVIDENCE (archive, 8/17): 210 `triggered_ma_pullback` rows, **123 distinct (ticker, price, ma)**, 35 tickers. YDES **40 rows at ONE price ($3.2933) over 34 minutes**, gaps 50/60/50/65/50/65…s = the scan cadence, not a market event. GRNQ 36 rows / 3 prices. `pullback_first_suppress` = 210, identical count, identical cause.
- **DID A DUPLICATE REACH A FILL? NO.** 8/17's four `filled` rows were FIEE (ignition), DFSC (ignition), NIVF (grinder), WFF (kevseq). Zero ma_pullback fills. On this day the defect cost COUNTS, not dollars — by luck, not design: the forty YDES passes were stopped by forty downstream rejections (themselves duplicated: ma_daily_bad 105, ma_low_room_soft 178), and had a gate flipped green mid-window nothing in the lane would have stopped a re-entry.
- **CORRECTED COUNTS: ma_pullback 210 rows -> at most 123 distinct (1.71x inflation).** Applied to the 592-fire study population (which counted ma_pullback at 193 — a 17-row discrepancy vs this pull, RECORDED not reconciled): **193 -> ~113, and the study total 592 -> ~512 distinct triggers**. ~13.5% of that population was one lane re-logging itself. Distinct-setup figures are an UPPER BOUND (two setups at the same price+MA collapse).
- C1 FIX: `_bar_epoch(b)` (junk -> 0 -> `_fire_once` never blocks); detector returns `k` = the CONFIRMATION CANDLE's epoch (the consumable unit); entry type 2 gated on `_fire_once("ma_pullback", …)` with refusals logged as `ma_pullback_dup_suppressed`; `_fire_seen()` NON-CONSUMING peek for the PULLBACK_FIRST pre-pass row (a consuming check there would have silenced the lane); `triggered_ma_pullback` now stamps `fire_k`. Pre-pass CONTROL FLOW (flat_top suppression) untouched. Kill: **MA_PULLBACK_DEDUPE=0**.
- **C2 HASH DEFINITION**: 12 hex of sha256 over (a) `code=<sha>` — RAILWAY_GIT_COMMIT_SHA/aliases, else the bot file's own digest, deliberately NOT the deploy id (a restart of the same image must NOT open an epoch, or the report is a restart counter); (b) `NAME=<value>` for every behaviour-governing env, name-sorted, UNSET spelled distinctly from set-to-empty. Env list **SCANNED FROM SOURCE** (`os.environ.get("NAME"` minus a non-behavioural exclusion set), ~180 vars, `cfg_n` stamped so a shrinking list is visible in data. STAMPS: boot_config + every `triggered_*` + every fill row (via the `_log_decision` choke point) + every trade record (via `post_trade_record_reliably`). NOT the ~6,700 ambient rows/day.
