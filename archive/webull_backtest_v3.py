#!/usr/bin/env python3
"""
Webull Deep Historical Backtest — v3
=====================================
v2 results: only 3 trades — 2.5% flat top + 1.5x vol was too restrictive.

Three changes from v2:
  1. FLAT_TOP_MAX_RANGE: 2.5% -> 3.5% (middle ground; 2.5% too tight for 1-min)
  2. BREAKOUT_VOL_MULT: 1.5x -> 1.2x (less aggressive)
  3. NEW: breakout bar must close in top 40% of its range (bullish close filter)
     This replaces raw vol spike as the quality gate — a bullish close means
     real momentum behind the breakout, not a wick.
...
# ── Strategy constants ────────────────────────────────────────────────────────
FLAT_TOP_WINDOW    = 8
FLAT_TOP_MAX_RANGE = 0.035  # <3.5% high-to-low range (v1=5%, v2=2.5%)
BREAKOUT_VOL_MULT  = 1.2    # breakout bar must have >=1.2x avg window volume
BULLISH_CLOSE_MIN  = 0.40   # breakout bar close must be in top 40% of bar range
MIN_ABS_VOL        = 10_000
EMA_PERIOD         = 9
TIME_CUTOFF        = "10:00"
MIN_PRICE          = 1.50
...
def _detect_flat_top(df: pd.DataFrame, i: int) -> bool:
    ...
    avg_window_vol = float(window["Volume"].mean())
    if avg_window_vol > 0 and vol < avg_window_vol * BREAKOUT_VOL_MULT:
        return False
    # Bullish close: close must be in the top 40% of the bar's range
    # (filters wicks and indecision candles — real breakouts close near the high)
    bar_high = float(df["High"].iloc[i])
    bar_low  = float(df["Low"].iloc[i])
    bar_range = bar_high - bar_low
    if bar_range > 0:
        close_position = (price - bar_low) / bar_range  # 0 = at low, 1 = at high
        if close_position < BULLISH_CLOSE_MIN:
            return False
    return True
...
def main():
    print(f"\n{'='*70}")
    print(f"  WEBULL DEEP BACKTEST v3 — Flat Top Breakout + EMA9 Stop")
    ...
    print(f"  Gap filter: {MIN_GAP_PCT*100:.0f}%–{MAX_GAP_PCT*100:.0f}% | "
          ...
          f"Vol spike: {BREAKOUT_VOL_MULT}x | Bullish close: top {BULLISH_CLOSE_MIN*100:.0f}%")
    print(f"{'='*70}\n")
...
def save_results(results):
    out = "/tmp/webull_backtest_v3_results.json"
    ...
    print(f"  (v3: flat top <3.5%, gap 15-30%, vol spike {BREAKOUT_VOL_MULT}x, bullish close top {BULLISH_CLOSE_MIN*100:.0f}%)")
