#!/usr/bin/env python3
"""
Webull Deep Historical Backtest — v9
=====================================
v8 results: 78 trades, 31% WR, 3.49:1 W/L, +$0.74 EV — best version so far.
Extending TIME_CUTOFF to 10:30 tripled trade count and nearly doubled EV vs v5.
Avg winner jumped $5.52 → $6.79; avg loser shrank $2.21 → $1.95.

One change from v8:
  1. TIME_CUTOFF: "10:30" → "11:00"  (Kev trades until 11am for a reason)
  All other v8 params unchanged: TARGET_PCT=0.10, FLAT_TOP_WINDOW=8
...
def main():
    print(f"\n{'='*70}")
    print(f"  WEBULL DEEP BACKTEST v9 — Flat Top Breakout + EMA9 Stop (2-bar confirm)")
...
def save_results(results):
    out = "/tmp/webull_backtest_v9_results.json"
    ...
    print(f"  (v9: flat top <5.0% 8-bar window, gap 15-30%, no vol filter, EMA9 {EMA_CONFIRM_BARS}-bar confirm, partial +{TARGET_PCT*100:.0f}%, cutoff {TIME_CUTOFF})")
