...
# F3 — FULL COMMIT AUDIT OF TONIGHT'S WORK (8/17, appended by agent F)

Generated by the scan mode built in F2:

```
python3 rig/spec_gate.py --scan aa6dab8~1..HEAD --markdown     # mainline, 29 commits
python3 rig/spec_gate.py --scan 1b5a8dc..agent/F --markdown    # agent F's branch, 2 commits
```

The convening had **two** known record incidents. This is the complete picture over every
commit from the start of the enforcement-gate work (`aa6dab8`, ENFORCEMENT GATE 1) through
HEAD, plus agent F's un-merged branch.

## The result

**31 commits scanned. 1 FLAGGED. No NEW unlabelled behaviour change was found.**

The single flag is `460dca5` — the already-known incident, the one the record repair
documents. Every other behaviour-changing commit tonight names an acceptance test. That is
a meaningful negative result: the concurrency hole produced **exactly one** unlabelled
money-behaviour change in 31 commits, not a pattern of them.

## How to read the columns

* **behaviour-changing?** — spec_gate's AST classifier: `marcos_trading_bot.py` /
  `screener_app.py` normalised with docstrings, comments and statement-level
  `print`/`_log_decision` calls stripped. Conservative by design (it errs toward YES).
* **Acceptance?** — the `Acceptance:` trailer in the commit message, if any.
* **proven at parent?** — **always "not run" in scan mode, by design.** Proving requires two
  worktree checkouts per commit and is far too slow over a range. A trailer here means
  *CLAIMED*, never *PROVEN*. Only `python3 rig/spec_gate.py <sha>` proves. **The convening
  must not read a filled Acceptance column as evidence the test was watched to fail.**
* **flagged** — behaviour-changing **and** no acceptance trailer. This is the exact signature
  of the 8/17 concurrency failure, whatever produced it (`add -A`, a shared path, `--amend`,
  or forgetfulness).

## The table

| commit | subject | behaviour-changing? | Acceptance? | proven at parent? | flagged |
|---|---|---|---|---|---|
| `aa6dab8b9c7e` | ENFORCEMENT GATE 1: lane completeness — seven properties per convertible lane, pinned ma | no (exempt) | — | n/a (exempt) |  |
| `ffc5f8354f8a` | ENFORCEMENT GATE 2: study provenance — killtest scripts may not hand-roll bot detectors | no (exempt) | — | n/a (exempt) |  |
| `4683a88dd555` | GATE 5: spec-as-failing-test — a behaviour-changing commit must name an acceptance test  | no (exempt) | — | n/a (exempt) |  |
| `56f8aec9eb3c` | ENFORCEMENT GATE 3: claims ledger — every stated fact carries the command that reproduce | no (exempt) | — | n/a (exempt) |  |
| `220e7c6b6b3f` | GATE 6: claim-without-check detector — token grounding against the turn's own tool evide | no (exempt) | — | n/a (exempt) |  |
| `279d9c811e49` | ENFORCEMENT GATE 4: artifact caveat surfacing + four-gate manifest | no (exempt) | — | n/a (exempt) |  |
| `d8f778574c6f` | GATE 7: decision-vs-deployed reconciler — every settled ruling becomes a command | no (exempt) | — | n/a (exempt) |  |
| `a17ab1c7e6f4` | GATE 8 + rig wiring: today's five defects pinned as permanent regression fixtures | no (exempt) | — | n/a (exempt) |  |
| `c388ab716638` | convene scope: enforcement gates 5-8 appended to PENDING_CONVENE_20260817 | no (exempt) | — | n/a (exempt) |  |
| `543888f2fb83` | A1 duplicate fires: fired-bucket high-water mark — a bucket may emit exactly once | YES | `rig/test_gates_20260817.py::SPEC_fire_hwm_dedupe` | not run (scan reports, does not prove) |  |
| `2d0a6cb11bfc` | B1 (EG1-a): kevseq fires at the FILL BAR'S CLOSE, not at the setup bar's HIGH | YES | `rig/test_batchB_20260817.py::SPEC_kevseq_fire_on_close` | not run (scan reports, does not prove) |  |
| `efcb86bb4101` | A2 fed-bucket provenance + A3 harness-parity threshold gate | YES | `rig/test_gates_20260817.py::SPEC_fed_bucket_stamps` | not run (scan reports, does not prove) |  |
| `bbe1d37239b1` | A4: verdict qualifiers on the 18 bare-verdict artifacts (analyses NOT rewritten) | no (exempt) | — | n/a (exempt) |  |
| `88f952366a36` | A5: RESULTS_LEDGER correction — 8/17 trigger counts inflated by restart replay | no (exempt) | — | n/a (exempt) |  |
| `6ee3fe22890c` | B2 (EG1-b): the shared fire-age suppressor ported to v2conv/grinder/bandpass/prevwap — A | YES | `rig/test_batchB_20260817.py::SPEC_lane_fire_age_suppresses` | not run (scan reports, does not prove) |  |
| `460dca5f9b30` | A-batch fixups: stamp position, EG4 negative control, and a clock-flaky rig section | YES | — | **NO TEST NAMED** | 🚩 **YES** |
| `5cd2d34738f4` | A-batch fixups: stamp position, EG4 negative control, and a clock-flaky rig section | no (exempt) | `rig/test_gates_20260817.py::SPEC_stamp_position` | not run (scan reports, does not prove) |  |
| `3fc2e1ab7eba` | convene scope: RELIABILITY BATCH B appended to PENDING_CONVENE_20260817 | no (exempt) | — | n/a (exempt) |  |
| `8ac67914c6d5` | RECORD REPAIR: two working-tree collisions with the concurrent B-batch agent (both mine) | no (exempt) | — | n/a (exempt) |  |
| `5d76d59d7865` | B5 correction: the armed volume guard refuses, it does NOT size to zero shares | YES | `rig/test_batchB_20260817.py::SPEC_fail_open_gates_observable_and_armable` | not run (scan reports, does not prove) |  |
| `649b68082a36` | convene scope: batch-B addendum — the second sweep collision recorded | no (exempt) | — | n/a (exempt) |  |
| `d3a5dd54ed1a` | RECORD REPAIR addendum: AST evidence — 460dca5 carried TWO unlabelled behaviour changes | no (exempt) | — | n/a (exempt) |  |
| `fb67828f4c06` | C1: ma_pullback consumes its setup — the within-process duplication closed | YES | `rig/test_gates_20260817.py::SPEC_ma_pullback_dedupe` | not run (scan reports, does not prove) |  |
| `3a7f881d0414` | C2: CONFIG HASH FREEZE — every fire, fill and trade record names its machine | YES | `rig/test_gates_20260817.py::SPEC_config_hash_stamp` | not run (scan reports, does not prove) |  |
| `f3c5fe57b091` | C2 follow-through: live_harness supplies hashlib + __file__ (the third ring) | no (exempt) | — | n/a (exempt) |  |
| `bbe419f7835f` | RESULTS_LEDGER: Foundation Batch C appended (C1 ma_pullback duplication, C2 config hash) | no (exempt) | — | n/a (exempt) |  |
| `ef0dfe5340f7` | D: BREAK-ATTACK EXTRACTED — our best-validated lane is testable at last | YES | `rig/test_batchD_20260817.py::SPEC_break_attack_extracted_and_live_calls_it` | not run (scan reports, does not prove) |  |
| `1b5a8dc93863` | D doc: LIMITS/CAVEATS section + an explicit qualified VERDICT (EG4 clean) | no (exempt) | — | n/a (exempt) |  |
| `49a4fd4df91e` | RECORD REPAIR incident 3: my doc commit 1b5a8dc swept three of the other agent's staged  | no (exempt) | — | n/a (exempt) |  |
| `9797563a94bb` | F1: GATE-BLIND ROWS — the three fail-open gates become countable per fire | YES | `rig/test_batchF_20260817.py::SPEC_gate_blind_row_is_attributable` | not run (scan reports, does not prove) |  |
| `1dd6165c78c6` | F1 doc: gate-blind rows — the 8/17 numbers, the 2000/day bound and why, and the read-bac | no (exempt) | — | n/a (exempt) |  |

Totals: **31 commits scanned, 1 FLAGGED.**

## Caveats the convening should carry

1. **Exemptions are not innocence.** `5cd2d34` reads *exempt* only because its parent is
   `460dca5` — the duplicate re-commit of the same content. The contaminated change is
   counted once, at `460dca5`, which is correct, but "exempt" here means "no delta versus a
   parent that was itself flagged", not "harmless".
2. **The scan cannot see contamination that carried a trailer.** If a swept-in change had
   ridden along in a commit that happened to name an acceptance test for its *own* change,
   the commit reads green. Nothing in the record range shows that, but the check has that
   blind spot and the fix for it is F2's per-agent worktrees, not a better scan.
3. **A THIRD incident occurred tonight, during the repair itself** (recorded in `49a4fd4`):
   agent D's doc commit `1b5a8dc` swept three of agent F's staged F2 files. It is *not*
   flagged, because those files are `rig/` and `data/` — no watched file, no behaviour
   change, so no acceptance test was owed. The record is wrong (the files sit under another
   agent's message) but no money behaviour moved. **Three incidents in one evening from the
   same root cause is the number the convening should weigh** when judging whether the
   per-agent worktree requirement in `data/audits/AGENT_WORKFLOW.md` is proportionate.
4. **Agent F's two commits are on branch `agent/F`, not merged.** `9797563` (F1 gate-blind
   rows) was proven through the full gate — `spec_gate.py 9797563` → PROVEN, fails at parent
   rc=1, passes at commit. It needs merging.
