...
def run_day(bars, emas, gaps):
    ...
    e9, e90 = ema(closes, 9), ema(closes, 90)
    cpv = cv = 0.0
    vw = []
    for b in bars:
        tp = (b["h"] + b["l"] + b["c"]) / 3.0
    ...
    for i in range(95, len(bars) - 3):
        ...
        if res["X_e3"] is not None:
            ...
            res["X_e3x"] = None
        busy_until = xi_cross if xi_cross else len(bars)
        out.append(res)
    return out
...
def main():
    ...
    W("=" * 100)
    W("THE 9/90 CROSS — Marcos's signal, tested as a TRIGGER (10s bars, 09:30-16:00 ET)")
    W("=" * 100)
    ...
    ARMS = ["X_cross", "X_e3", "X_e3x", "X_hold"]
    ...
    json.dump({"out": OUT}, open(HERE + "/ema9x90_cross_20260818_out.json", "w"))
    return 0
