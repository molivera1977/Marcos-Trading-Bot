...
def run_day(bars, emas, gaps):
    ...
    out = []
    busy_until = -1
    # 8/18 (Marcos: "try the one minute time frame") — the ENTRY cross is now read on 1-MINUTE
    # bars, not 10s. A 10s 9/90 cross fired 17,330 times across the cache (~275/day) which is
    # noise-rate; the 1-min cross is a far rarer, more meaningful regime change. Execution still
    # happens on the 10s bar that CLOSES the crossing minute, so fills stay realistic.
    M1 = SLOW[6]
    m1_cross = set()
    for a in range(1, len(M1["idx"])):
        if M1["e9"][a - 1] <= M1["e90"][a - 1] and M1["e9"][a] > M1["e90"][a]:
            m1_cross.add(M1["idx"][a])
    for i in range(95, len(bars) - 3):
        if i <= busy_until:
            continue
        if i not in m1_cross:
            continue
        hh = E.hhmm_b(bars[i])
        ...
        # ── 8/18: a lane needs a STOP, not just an exit rule. X_hold/X_1m20 measured DRIFT
        # with unbounded downside; nothing ships on that. Three candidate stops, each a real
        # structural line available at entry:
        #   S_e90   the 1-min 90-EMA being crossed (the line whose break invalidates the setup)
        #   S_swing the lowest low of the 30 10s bars before the cross (the swing the move left)
        #   S_pct   a flat 4% — the dumb control; if it wins, the structure ones are noise
        _m1 = SLOW[6]
        _ai = max((a for a in range(len(_m1["idx"])) if _m1["idx"][a] <= i), default=0)
        STOPS = {"S_e90": _m1["e90"][_ai],
                 "S_swing": min(b["l"] for b in bars[max(0, i - 30):i + 1]),
                 "S_pct": bars[i]["c"] * 0.96}
        ...
        sh = E.POS / entry
        _m1b = SLOW[6]
        _aa = max((a for a in range(len(_m1b["idx"])) if _m1b["idx"][a] <= i), default=0)
        _sess_hi = max(b["h"] for b in bars[:i + 1])
        _sess_lo = min(b["l"] for b in bars[:i + 1])
        _open = bars[0]["o"]
        res = {"i": i, "above_vwap": above_vwap,
               "vwap_dist": (bars[i]["c"] / vw[i] - 1) * 100.0,
               "vwap_slope": (vw[i] - vw[max(0, i - 30)]) / max(vw[max(0, i - 30)], 1e-9) * 100.0,
               "from_hi": (bars[i]["c"] / _sess_hi - 1) * 100.0,
               "off_lo": (bars[i]["c"] / max(_sess_lo, 1e-9) - 1) * 100.0,
               "day_gain": (bars[i]["c"] / max(_open, 1e-9) - 1) * 100.0,
               "hhmm": E.hhmm_b(bars[i])[:5],
               "e20_over_e90": _m1b["e20"][_aa] > _m1b["e90"][_aa],
               # ── VELOCITY (Marcos: "how about velocity") — how HARD is it crossing?
               # A dead-cat bounce drifts across the 90; a real reclaim rips through it.
               # Measured five ways at the cross bar, all causal (no future bars):
               "vel_1m": (bars[i]["c"] / max(bars[max(0, i - 6)]["c"], 1e-9) - 1) * 100.0,
               "vel_3m": (bars[i]["c"] / max(bars[max(0, i - 18)]["c"], 1e-9) - 1) * 100.0,
               "gap_pct": (_m1b["e9"][_aa] / max(_m1b["e90"][_aa], 1e-9) - 1) * 100.0,
               "e9_slope": ((_m1b["e9"][_aa] - _m1b["e9"][max(0, _aa - 3)])
                            / max(_m1b["e9"][max(0, _aa - 3)], 1e-9) * 100.0),
               "volx": (sum(b["v"] for b in bars[max(0, i - 6):i + 1])
                        / max(sum(b["v"] for b in bars[max(0, i - 96):i - 6]) / 15.0, 1e-9)),
        ...
        # ── 8/18 (Marcos: "so what are your exits, they didn't look good") — he is right:
        # above VWAP, HOLD makes +$55.00/tr and the best exit rule makes +$12.82. Every exit
        # gives back ~77% of the edge. The untested middle: BOUND THE DOWNSIDE AND HOLD.
        #   H_*    stop only, otherwise hold to session end  (no exit rule at all)
        #   T_e9   trail the 1-min 9-EMA (exit on a 1-min CLOSE below it, not a cross)
        #   B_half bank half at +10%, hold the rest to the close behind the stop
        for _sn, _sv in STOPS.items():
            if _sv >= entry:
                res["H_" + _sn] = None; continue
            _hit = None
            for k2 in range(i + 1, len(bars)):
                if bars[k2]["l"] <= _sv:
                    _hit = k2; break
            res["H_" + _sn] = (sh * (_sv * (1 - MKT) - entry) if _hit is not None
                               else sh * (bars[-1]["c"] * (1 - MKT) - entry))
        _sw = STOPS["S_swing"]
        _m1t = SLOW[6]
        _tx = None
        for a in range(len(_m1t["idx"])):
            if _m1t["idx"][a] <= i: continue
            if _m1t["c"][a] < _m1t["e9"][a]:
                _tx = _m1t["idx"][a]; break
        _stop_first = None
        for k2 in range(i + 1, (_tx if _tx is not None else len(bars))):
            if bars[k2]["l"] <= _sw:
                _stop_first = k2; break
        if _sw >= entry:
            res["T_e9"] = None
        elif _stop_first is not None:
            res["T_e9"] = sh * (_sw * (1 - MKT) - entry)
        else:
            _px = bars[_tx]["c"] if _tx is not None else bars[-1]["c"]
            res["T_e9"] = sh * (_px * (1 - MKT) - entry)
        # bank half at +10%, rest to the close, stop live on the remainder
        if _sw < entry:
            _tgt = entry * 1.10; _pnl = 0.0; _rem = sh; _scaled = False
            for k2 in range(i + 1, len(bars)):
                b2 = bars[k2]
                if b2["l"] <= _sw:
                    _pnl += _rem * (_sw * (1 - MKT) - entry); _rem = 0; break
                if not _scaled and b2["h"] >= _tgt:
                    _pnl += (sh * 0.5) * (_tgt - entry); _rem -= sh * 0.5; _scaled = True
            if _rem > 0:
                _pnl += _rem * (bars[-1]["c"] * (1 - MKT) - entry)
            res["B_half"] = _pnl
        else:
            res["B_half"] = None
        # stopped versions of the winning exit (1-min 9/20): stop-first INTRABAR, honest.
        for _sn, _sv in STOPS.items():
            if _sv >= entry:
                res["L_" + _sn] = None
                continue
            _p, _hit = None, False
            for k2 in range(i + 1, min(j1 + 1, len(bars))):
                if bars[k2]["l"] <= _sv:
                    _p = sh * (_sv * (1 - MKT) - entry); _hit = True; break
            res["L_" + _sn] = _p if _hit else sh * (p1 - entry)
        try:
...
def main():
    ...
    W("=" * 100)
    W("THE 1-MIN 9/90 LANE — entry 1m cross, exit 1m 9/20, STOPPED (the shippable form)")
    W("=" * 100)
    ...
    ARMS = ["H_S_swing", "H_S_e90", "H_S_pct", "T_e9", "B_half", "L_S_swing", "X_1m20", "X_hold"]
    ...
    json.dump({"out": OUT, "rows": rows}, open(HERE + "/ema9x90_exits_20260818_out.json", "w"))
    return 0
