...
def fires_for(bars):
    ...
    for a in range(1, len(i1)):
        ...
        # ── 8/18 (Marcos: "i just can't understand how you can't figure out a working exit
        # that is better than waiting until close") — the arms tried so far all trailed a fast
        # line and capped the tail. UNTESTED until now, and it is KEV'S ACTUAL RULE: the trade
        # is over when it LOSES VWAP. Far wider leash than any MA cross. Plus wide off-high
        # trails, which keep the runner but bound the give-back.
        for _nm, _stop in (("pct4", bars[i]["c"] * 0.96),
                           ("swing", min(b["l"] for b in bars[max(0, i - 30):i + 1]))):
            if _stop >= entry:
                for _k in ("vwapx_", "off15_", "off25_"):
                    rec[_k + _nm] = None
                continue
            # exit on a 1-min CLOSE below session VWAP (stop still live, first one wins)
            _ex = None
            for k2 in range(i + 1, len(bars)):
                if bars[k2]["l"] <= _stop:
                    _ex = ("stop", k2); break
                if (k2 - i) % 6 == 0 and bars[k2]["c"] < vw[k2]:
                    _ex = ("vwap", k2); break
            if _ex is None:
                rec["vwapx_" + _nm] = sh * (bars[-1]["c"] * (1 - MKT) - entry)
            elif _ex[0] == "stop":
                rec["vwapx_" + _nm] = sh * (_stop * (1 - MKT) - entry)
            else:
                rec["vwapx_" + _nm] = sh * (bars[_ex[1]]["c"] * (1 - MKT) - entry)
            # wide off-high trails: keep the runner, bound the give-back
            for _tag, _fr in (("off15_", 0.85), ("off25_", 0.75)):
                _hi = entry; _out = None
                for k2 in range(i + 1, len(bars)):
                    if bars[k2]["l"] <= _stop:
                        _out = sh * (_stop * (1 - MKT) - entry); break
                    _hi = max(_hi, bars[k2]["h"])
                    if bars[k2]["c"] < _hi * _fr:
                        _out = sh * (bars[k2]["c"] * (1 - MKT) - entry); break
                rec[_tag + _nm] = _out if _out is not None else sh * (bars[-1]["c"] * (1 - MKT) - entry)
...
def main():
    ...
    W("FULL SAMPLE — above VWAP   (HOLD = stop only, no exit rule)")
    for a in ("pct4", "swing", "vwapx_pct4", "vwapx_swing",
              "off15_pct4", "off15_swing", "off25_pct4", "off25_swing"):
        line(a, stat(ab, a))
    ...
    for lbl, tr, ho in [
        ...
        for a in ("pct4", "swing", "vwapx_swing", "off15_swing", "off25_swing"):
    ...
    json.dump({"out": OUT}, open(HERE + "/ema9x90_wall_v2_20260818_out.json", "w"))
    return 0
