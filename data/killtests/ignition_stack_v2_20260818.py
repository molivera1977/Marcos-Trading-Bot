...
def main():
    ...
    for (sym, date), (bars, emas, gaps) in E.DAYS.items():
        ...
        for f in fs:
            ...
            # 8/18 FIX (Marcos: "why can't it run"). It can — I built the 1-min series off the
            # RTH-only slice (E.DAYS), so a 90-EMA needed 90 minutes of RTH and essentially no
            # ignition fire qualified: the diagnostic printed s920/s990 = None on all 3,531 fires
            # and the four arms were four copies of the no-stack arm. S.FULL carries the WHOLE
            # day incl. premarket (5,597 bars vs 2,312 RTH on the sample day = 328 premarket
            # 1-min bars), which is also what the live detector reads (cache[t]["full_bars"]).
            # Warmed from premarket, the 90-EMA is live from the opening bell.
            _fb = S.FULL.get((sym, date)) or bars
            _cut = next((n for n, b in enumerate(_fb) if b["t"] >= bars[i]["t"]), len(_fb) - 1)
            _c1 = [_fb[a + 5]["c"] for a in range(0, max(0, _cut - 5), 6)]
            if len(_c1) >= 91:
                def _e(v, n):
                    k = 2.0 / (n + 1); e = None
