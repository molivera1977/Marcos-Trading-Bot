...
def main():
    ...
    for (sym, date), (bars, emas, gaps) in E.DAYS.items():
        ...
        for f in ig:
            ...
            try:
                ...
                continue
            fires.append({"sym": sym, "date": date, "i": i, "lane": "IGN", "pnl": pnl})
        # 9/90
        c1, i1 = [], []
        ...
        for a in range(1, len(i1)):
            ...
            fires.append({"sym": sym, "date": date, "i": i, "lane": "X9", "pnl": pnl})
    ...
    W("\nCAPACITY — dollars per day, first N fires per day (hold-out)")
