#!/usr/bin/env python3
"""
VWAP_RECLAIM — THE WALL + CONSTRUCTION SWEEP (kev_reclaim_step, 3-gate grammar) (8/18)
...
This script holds the DETECTIONS FIXED (the same fires, the bot's own kev_reclaim_step via
...
def main():
    ...
    W("=" * 104)
    W("VWAP_RECLAIM — THE WALL + CONSTRUCTION SWEEP (kev_reclaim_step, 3-gate grammar)")
    W("=" * 104)
    W(f"universe: {len(days)} name-days / {len(dates)} dates. Detections held FIXED "
      f"(bot's own kev_reclaim_step); everything downstream swept.\n")
    ...
    json.dump({"out": OUT}, open(HERE + "/reclaim_wall_20260818_out.json", "w"))
    return 0
