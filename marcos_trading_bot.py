...
#   chart either — see the front-side SOURCE AUDIT block below and
...
# kevseq has always been 1-min-gated.  NO behaviour switch was added for a defect that does
# not exist — this block is documentation + STAMPS only.
# WHAT THE CANARY ACTUALLY CATCHES — two 1-MIN sources on DIFFERENT CLOCKS (reconstructed from
# the SIP tape for all 31 rows: data/killtests/kevseq_frontside_sources_20260817.py, caller sign
# reproduced 31/31):
#   caller = the M1 REST fetch = TRADED MINUTES ONLY, count-capped at 50, RTH-filtered.  On a
#            thin name its 49 "1-minute bars" stretch backwards over HOURS: UUU 8/17 = 584 min
#            (9.7h) of wall clock, RBNE 233-260 min, FXHO 183 min.  Its "20-bar EMA" is then a
#            multi-hour trend wearing a 1-minute label.
#   self   = kevseq_feed_1m over the fed 10s stream = a CONTIGUOUS wall-clock minute grid
#            (quiet minutes still produce a bucket), day-wide incl. premarket.  UUU held 155
#            buckets on a day whose whole SIP tape has 54 traded minutes.
#   On liquid names the two clocks coincide (span == bar count) and the residual disagreements
#   are knife-edge: |EMA9-EMA20| <= 0.2% on every such row.
# WHICH SOURCE GOVERNS IS A MONEY DECISION -> Marcos's call.  Live precedence is UNCHANGED
# (caller first, self only when the caller is short).  The 3-min setup-chart value is STAMPED
# as front_side_3m (counterfactual only, governs nothing), and BOTH 1-min values are stamped on
# every row (front_side_caller / front_side_self) so the decision can be graded from the archive.
                    ...
                    if _nb:
                        ...
                        if KEVSEQ_SHADOW:
                            try:
                                ...
                                _ks_fs_caller, _ks_self_fs, _ks_self_n = None, None, None
                                try:
                                    # 8/17: the SETUP_TF_MIN (3-min) front side is computed here
                                    # for the STAMP ONLY — nothing reads it back into ctx. This is
                                    # the 1m-vs-3m counterfactual made auditable at zero behaviour
                                    # cost (the alleged 3-min gate never existed; see the block above).
                                    _ks_c3 = []
                                    try:
                                ...
                                if _ksf:
                                    ...
                                    _ks_row = dict(price=_ksf["px"], seq_str=_ksf["seq_str"], kind=_ksf["kind"],
                                                   ...
                                                   front_side_3m=_ks_fs_3m, front_side_tf=1,
                                                   front_side_caller=_ks_fs_caller, front_side_self=_ks_self_fs,
                                                   front_side_self_n=_ks_self_n,
