...
check("W6 extension-guard exempt: rocket-class + slow lanes + curls (Marcos 7/26)",
      '"rocket_catcher", "hidden_entry", "flat_top", "orb", "ma_pullback",' in BOT
      and '"vwap_reclaim", "zone_flip"' in BOT)
...
check("W18 premarket cap 6 + $250k dvol floor", bot.PRE_MAX_TRADES == 6 and abs(bot.PRE_MIN_DVOL - 250000) < 1)
check("W21 premarket 9:25 flatten wired", bot.PRE_FLAT_HHMM == "09:25" and "_entered_premkt" in BOT and "premarket time stop" in BOT)
check("W22 flatten scoped: PRE-entered trades only, window-bounded", '_entered_premkt and now.strftime("%H:%M") >= PRE_FLAT_HHMM' in BOT)
...
check("W14 vel5/daygain legacy lists exclude hidden (passes)", '"hidden_entry"' not in BOT.split('b[3] in ("flat_top", "ma_pullback", "orb", "ema_bounce")')[0][-200:])
