#!/usr/bin/env python3
"""
Webull Deep Historical Backtest — v2
=====================================
v1 results: 29% WR, -$0.48 EV/trade.  Three targeted fixes:
  1. Volume spike required on breakout bar (>=1.5x avg window vol)
  2. Tighter flat top range: 2.5% (was 5%) — cleaner consolidations only
  3. Max gap capped at 30% (30-60% bucket was 0% WR in v1)
...
# ── Strategy constants ────────────────────────────────────────────────────────
FLAT_TOP_WINDOW    = 8
FLAT_TOP_MAX_RANGE = 0.025  # <2.5% high-to-low range (was 5%) — tighter consolidation
BREAKOUT_VOL_MULT  = 1.5    # breakout bar must have >=1.5x avg window volume
MIN_ABS_VOL        = 10_000
EMA_PERIOD         = 9
TIME_CUTOFF        = "10:00"
MIN_PRICE          = 1.50
...
def _detect_flat_top(df: pd.DataFrame, i: int) -> bool:
    ...
    if vol < MIN_ABS_VOL:
        return False
    # Volume spike: breakout bar must be significantly busier than the consolidation
...
def main():
    print(f"\n{'='*70}")
    print(f"  WEBULL DEEP BACKTEST v2 — Flat Top Breakout + EMA9 Stop")
    ...
    print(f"  Gap filter: {MIN_GAP_PCT*100:.0f}%–{MAX_GAP_PCT*100:.0f}% | "
          ...
          f"Vol spike: {BREAKOUT_VOL_MULT}x")
    print(f"{'='*70}\n")
...
def save_results(results):
    out = "/tmp/webull_backtest_v2_results.json"
    ...
    print(f"  (v2: flat top <2.5%, gap 15-30%, vol spike {BREAKOUT_VOL_MULT}x)")
