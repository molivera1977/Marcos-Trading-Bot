#!/usr/bin/env python3
"""
Webull Deep Historical Backtest — v4
=====================================
v3 results: 7 trades, 14% WR (1W/6L) — worst result yet, EV = -$0.60/trade.
The bullish close filter specifically eliminated NVNI 2025-01-17 (the main
winner across v1/v2) and let through 4 new losers. All exits are EMA STOP;
the +10% partial target has never triggered across 41 total trades.

Two changes from v3:
  1. FLAT_TOP_MAX_RANGE: 3.5% -> 5.0% (back to v1 — 3.5% still too tight)
  2. Removed BULLISH_CLOSE_MIN filter (proven harmful — eliminated winners)
  Keeping BREAKOUT_VOL_MULT = 1.2 (mild vol quality gate, only addition vs v1)
...
def main():
    print(f"\n{'='*70}")
    print(f"  WEBULL DEEP BACKTEST v4 — Flat Top Breakout + EMA9 Stop")
    ...
    print(f"  Gap filter: {MIN_GAP_PCT*100:.0f}%–{MAX_GAP_PCT*100:.0f}% | "
          ...
          f"Vol spike: {BREAKOUT_VOL_MULT}x | No bullish close filter")
    print(f"{'='*70}\n")
...
def save_results(results):
    out = "/tmp/webull_backtest_v4_results.json"
    ...
    print(f"  (v4: flat top <5.0%, gap 15-30%, vol spike {BREAKOUT_VOL_MULT}x, no bullish close filter)")
