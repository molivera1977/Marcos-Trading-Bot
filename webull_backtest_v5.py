#!/usr/bin/env python3
"""
Webull Deep Historical Backtest — v5
=====================================
v4 results: 19 trades, 21% WR (4W/15L), EV = -$1.08/trade.
Adding the 1.2x volume filter (v4 vs v1) eliminated 12 trades and made
WR and W/L ratio WORSE — the volume filter is anti-correlated with winners.
Also, 0/48+ trades have ever hit the +10% partial target across all versions;
the EMA9 stop is being triggered by single-bar wicks on volatile small-caps.

Two changes from v4:
  1. Removed BREAKOUT_VOL_MULT filter (returns to v1 baseline — vol hurts)
  2. EMA9 stop now requires 2 consecutive closes below EMA9 before exiting
     (reduces whipsaw exits from single wicks)
...
def main():
    print(f"\n{'='*70}")
    print(f"  WEBULL DEEP BACKTEST v5 — Flat Top Breakout + EMA9 Stop (2-bar confirm)")
    ...
    print(f"  Gap filter: {MIN_GAP_PCT*100:.0f}%–{MAX_GAP_PCT*100:.0f}% | "
          ...
          f"No vol filter | EMA stop: {EMA_CONFIRM_BARS}-bar confirm | No bullish close filter")
    print(f"{'='*70}\n")
...
def save_results(results):
    out = "/tmp/webull_backtest_v5_results.json"
    ...
    print(f"  (v5: flat top <5.0%, gap 15-30%, no vol filter, EMA9 {EMA_CONFIRM_BARS}-bar confirm stop)")
