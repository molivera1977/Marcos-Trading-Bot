#!/usr/bin/env python3
"""
Webull Deep Historical Backtest — v7
=====================================
v6 results: 9 trades, 22% WR, -$1.32 EV — regression from v5.
The 12-bar flat top window (v6) was too restrictive for volatile small-cap
gappers: eliminated 18 of 27 v5 signals, keeping only weaker exhausted setups.
These stocks establish flat tops in ~8 bars and move; forcing 12 bars misses
the breakout entirely.

One change from v6 (reverting v6's bad change, keeping v6's good change):
  1. FLAT_TOP_WINDOW: 12 → 8  (back to v5 baseline — 12 bars proved too slow)
  2. TARGET_PCT stays at 0.05  (5% partial untested in isolation — test it now)
...
def main():
    print(f"\n{'='*70}")
    print(f"  WEBULL DEEP BACKTEST v7 — Flat Top Breakout + EMA9 Stop (2-bar confirm)")
...
def save_results(results):
    out = "/tmp/webull_backtest_v7_results.json"
    ...
    print(f"  (v7: flat top <5.0% 8-bar window, gap 15-30%, no vol filter, EMA9 {EMA_CONFIRM_BARS}-bar confirm, partial +{TARGET_PCT*100:.0f}%)")
