#!/usr/bin/env python3
"""
Webull Deep Historical Backtest — v8
=====================================
v7 results: 27 trades, 37% WR, 2.00:1 W/L, +$0.26 EV — lower than v5.
The 5% partial exit (v7) cut avg winners from ~$5.52 → $4.64 vs v5's 10%.
One extra win (37% vs 33% WR) wasn't enough to offset the lower W/L ratio.
v5 remains the best at +$0.37 EV.

One change from v5 baseline:
  1. TIME_CUTOFF: "10:00" → "10:30"  (30 more minutes to form flat tops)
  All other v5 params restored: TARGET_PCT=0.10, FLAT_TOP_WINDOW=8
...
def main():
    print(f"\n{'='*70}")
    print(f"  WEBULL DEEP BACKTEST v8 — Flat Top Breakout + EMA9 Stop (2-bar confirm)")
...
def save_results(results):
    out = "/tmp/webull_backtest_v8_results.json"
    ...
    print(f"  (v8: flat top <5.0% 8-bar window, gap 15-30%, no vol filter, EMA9 {EMA_CONFIRM_BARS}-bar confirm, partial +{TARGET_PCT*100:.0f}%, cutoff {TIME_CUTOFF})")
